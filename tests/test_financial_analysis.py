...
from financial_ai.analysis.fundamentals import FinancialPeriod, StatementValue, analyze_financial_quality
...
def test_financial_quality_converts_units_and_displays_decimal_percentages_correctly() -> None:
    periods = [
        FinancialPeriod("2024", revenue=usd_millions(100), net_income=usd_millions(15), equity=usd_millions(50), invested_capital=usd_millions(90), diluted_shares=StatementValue(100, "shares")),
        FinancialPeriod("2025", revenue=usd_millions(139), net_income=usd_millions(20), eps=StatementValue(2, "per_share"), free_cash_flow=usd_millions(30), gross_profit=usd_millions(54.21), operating_income=usd_millions(30), effective_tax_rate=0.21, equity=usd_millions(60), invested_capital=usd_millions(100), current_assets=usd_millions(80), current_liabilities=usd_millions(40), total_debt=usd_millions(30), diluted_shares=StatementValue(105, "shares"), stock_compensation=usd_millions(4), capex=usd_millions(-10)),
    ]
...
def test_valuation_does_not_turn_negative_denominators_into_misleading_ratios() -> None:
    current = analyze_valuation(ValuationInput(market_cap=1_000, enterprise_value=1_100, net_income=-20, free_cash_flow=-5, revenue=200, equity=-1, ebitda=0))
    ...
    ranges = historical_ranges([current, analyze_valuation(ValuationInput(market_cap=1_000, revenue=100))])
...
def test_peer_universe_manual_overrides_and_percentiles_are_deterministic() -> None:
    universe = PeerUniverse("same industry and US-listed market-cap range", ("MSFT", "GOOG"), ("META",), ("GOOG",))
    assert universe.symbols() == ("MSFT", "META")
    target = PeerCompany("AAPL", "Apple", "Technology", "Hardware", {"revenue_growth": 0.2, "pe": 25})
    peers = [PeerCompany("MSFT", "Microsoft", "Technology", "Software", {"revenue_growth": 0.1, "pe": 30}), PeerCompany("META", "Meta", "Technology", "Internet", {"revenue_growth": 0.3, "pe": 20})]
    results = {item.metric: item for item in compare_company(target, peers, ("revenue_growth", "pe", "missing"))}
