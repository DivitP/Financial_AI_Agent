...
def _ratio(name: str, numerator: float | None, denominator: float | None, *, unit: Literal["percent", "multiple"] = "percent") -> MetricResult:
...
def analyze_financial_quality(periods: list[FinancialPeriod]) -> dict[str, MetricResult]:
    ...
    revenue, prior_revenue = _amount(current.revenue), _amount(previous.revenue) if previous else None
    fcf, prior_fcf = _amount(current.free_cash_flow), _amount(previous.free_cash_flow) if previous else None
    capex = _amount(current.capex)
    eps = current.eps.value if current.eps and current.eps.unit == "per_share" else None
    prior_eps = previous.eps.value if previous and previous.eps and previous.eps.unit == "per_share" else None
    average_equity = _average(_amount(current.equity), _amount(previous.equity) if previous else None)
    average_capital = _average(_amount(current.invested_capital), _amount(previous.invested_capital) if previous else None)
    operating_income = _amount(current.operating_income)
    nopat = operating_income * (1 - current.effective_tax_rate) if operating_income is not None and current.effective_tax_rate is not None else None
    return {
        "revenue": MetricResult("revenue", revenue, "currency", None if revenue is not None else "revenue is unavailable"),
        "eps": MetricResult("eps", eps, "per_share", None if eps is not None else "EPS per-share data is unavailable"),
        "free_cash_flow": MetricResult("free_cash_flow", fcf, "currency", None if fcf is not None else "free cash flow is unavailable"),
        ...
        "current_ratio": _ratio("current_ratio", _amount(current.current_assets), _amount(current.current_liabilities), unit="multiple"),
        "debt_to_equity": _ratio("debt_to_equity", _amount(current.total_debt), _amount(current.equity), unit="multiple"),
        "dilution_growth": _growth("dilution_growth", _shares(current.diluted_shares), _shares(previous.diluted_shares) if previous else None),
        "stock_compensation_to_revenue": _ratio("stock_compensation_to_revenue", _amount(current.stock_compensation), revenue),
        "capex_to_revenue": _ratio("capex_to_revenue", abs(capex) if capex is not None else None, revenue),
