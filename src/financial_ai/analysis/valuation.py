...
def _multiple(name: str, numerator: float | None, denominator: float | None) -> MetricResult:
    ...
    if denominator <= 0:
        return MetricResult(name, None, "multiple", "denominator is zero or negative; ratio is not meaningful")
...
def _yield(name: str, numerator: float | None, denominator: float | None) -> MetricResult:
    ...
    if numerator <= 0 or denominator <= 0:
        return MetricResult(name, None, "percent", "earnings or cash flow is zero or negative; yield is not meaningful")
...
def analyze_valuation(value: ValuationInput) -> dict[str, MetricResult]:
    return {
        ...
        "free_cash_flow_yield": _yield("free_cash_flow_yield", value.free_cash_flow, value.market_cap),
...
def historical_ranges(history: list[dict[str, MetricResult]]) -> dict[str, ValuationRange]:
    ...
    return {
        name: _range(name, [snapshot[name].value for snapshot in history if snapshot.get(name) and snapshot[name].value is not None])
        for name in names
    }
...
def _range(name: str, values: list[float | None]) -> ValuationRange:
    ...
    return ValuationRange(name, min(valid) if valid else None, median(valid) if valid else None, max(valid) if valid else None, len(valid))
